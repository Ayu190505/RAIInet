...
int getPlayerIndex(int player, vector<int> turns) {for (int i = 0; i < turns.size(); ++i) if (turns[i] == player) return i;}
...
void TextObserver::player_line_void() {out << std::string(center, ' ');}
